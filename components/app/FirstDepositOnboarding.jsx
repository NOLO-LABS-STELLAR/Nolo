  ...
  return (
    <div className="vq-glass relative overflow-hidden rounded-3xl border border-nolo-500/20 bg-[#1A120D]/40 p-6 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="relative flex items-start justify-between border-b border-vault-border/30 pb-4 mb-6">
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-full bg-nolo-500/10 px-3 py-1 text-xs font-semibold text-nolo-400">
      ...
      <div className="relative grid gap-6 md:grid-cols-2 lg:grid-cols-4 mb-8">
        ...
        <div className={`p-5 rounded-2xl border transition-all ${isConnected ? "border-emerald-500/20 bg-emerald-950/10" : "border-vault-border/40 bg-vault-surface/20"}`}>
          <div className="flex justify-between items-start">
            <span className={`flex h-10 w-10 items-center justify-center rounded-xl ${isConnected ? "bg-emerald-500/10 text-emerald-400" : "bg-nolo-500/10 text-nolo-400"}`}>
            ...
            {isConnected ? (
              ...
              <span className="text-[10px] uppercase font-bold text-nolo-400 px-2 py-0.5 rounded-full bg-nolo-500/10">Required</span>
            )}
          </div>
          ...
          <p className="text-xs text-vault-muted mt-1.5 leading-relaxed">
            Nolo uses your wallet to verify your savings balance and sign transactions.
          </p>
          {!isConnected && (
            <button
              type="button"
