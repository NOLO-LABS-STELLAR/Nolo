...
export const ActivityExport: FC<ActivityExportProps> = ({
  ...
  return (
    <section aria-label="Export activity" className="rounded-2xl border border-nolo-900/30 bg-[#1A120D]/60 p-5">
      ...
      {summary && (
        <dl className="mt-4 grid grid-cols-3 gap-3 rounded-xl border border-nolo-900/20 bg-black/20 p-3 text-center text-xs">
          <div>
      ...
      <div className="mt-4 flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-1">
          ...
          <input id="export-from" type="date" value={from} onChange={(e) => { reset(); setFrom(e.target.value); }}
            className="rounded-lg border border-nolo-900/30 bg-[#0D0101] px-3 py-2 text-sm text-white placeholder-gray-600 focus:outline-none focus:ring-2 focus:ring-nolo-500" />
        </div>
        <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select id="export-format" value={format} onChange={(e) => { reset(); setFormat(e.target.value as ExportFormat); }}
            className="rounded-lg border border-nolo-900/30 bg-[#0D0101] px-3 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-nolo-500">
      ...
      {state.status === "error" && (
        <div role="alert" className="mt-3 flex items-center gap-2 rounded-lg bg-nolo-700/20 px-3 py-2 text-sm text-nolo-300">
