  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={-1}
        className={`bg-[#1A120D] border border-nolo-900/40 rounded-2xl shadow-2xl max-w-md w-full relative overflow-hidden animate-in zoom-in-95 duration-300 outline-none max-h-[90vh] flex flex-col ${className}`}
      >
        <div className="p-6 sm:p-8 overflow-y-auto max-h-[calc(90vh-3rem)]">{children}</div>
        <button
          type="button"
          className="absolute top-4 right-4 text-gray-500 hover:text-white transition-colors p-2 hover:bg-white/10 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nolo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-[#1A120D] z-10"
          onClick={onClose}
