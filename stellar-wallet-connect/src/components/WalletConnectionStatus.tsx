...
function DisconnectedState({ onConnect, connectionError }: { onConnect?: () => void; connectionError?: string | null }) {
  return (
    <div className="mt-4 space-y-3">
      {connectionError ? (
        <div role="alert" className="flex items-start gap-2 rounded-lg bg-nolo-700/20 px-3 py-2 text-sm text-nolo-300">
      ...
      {onConnect && (
        <button type="button" onClick={onConnect} className="inline-flex items-center gap-2 rounded-xl bg-nolo-600 px-4 py-2 text-sm font-semibold text-white transition-colors hover:bg-nolo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-nolo-400">
...
export const WalletConnectionStatus: FC<WalletConnectionStatusProps> = ({
  ...
  return (
    <aside aria-label="Wallet connection status" className="rounded-2xl border border-nolo-900/30 bg-[#1A120D]/60 p-4 sm:p-5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          <span className={`flex h-10 w-10 items-center justify-center rounded-full ${isConnected ? "bg-emerald-900/30" : "bg-nolo-900/30"}`}>
            {isConnected
              ? <Wallet className="h-5 w-5 text-emerald-300" aria-hidden="true" />
              : <WifiOff className="h-5 w-5 text-nolo-300" aria-hidden="true" />}
          </span>
          <div>
        ...
        {isConnected && onReconnect && (
          <button type="button" onClick={onReconnect} disabled={isConnecting} aria-label="Reconnect wallet"
            className="inline-flex items-center gap-1.5 rounded-lg border border-nolo-500/30 px-3 py-1.5 text-xs font-medium text-gray-300 transition-colors hover:bg-nolo-900/30 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={handleCopy} aria-label="Copy wallet address"
              className="rounded p-1 text-gray-400 transition-colors hover:bg-nolo-900/20 hover:text-white focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-nolo-400">
          ...
          {onDisconnect && (
            <button type="button" onClick={onDisconnect} className="mt-1 text-xs text-gray-500 underline transition-colors hover:text-nolo-400">
              Disconnect
            </button>
          )}
        </div>
